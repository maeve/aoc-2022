...
actual_free_space = total_disk_space - root.size
space_to_free = required_free_space - actual_free_space

puts "Actual free space: #{actual_free_space}"
puts "Space to free: #{space_to_free}"

dirs = root.dirs.select { |d| d.size >= space_to_free }.sort_by(&:size)
puts 'Dirs:'
puts dirs.map(&:to_s)

puts "Answer: #{dirs.first.size}"
